...
class Minesweeper(Board):
    ...
    def find_num_for_cell(self, cell):
        x, y = cell
        n = 0
        for v in range(-1, 2):
            for k in range(-1, 2):
    ...
    def open_cell(self, cell):
        ...
        x, y = cell
        lst1 = list()
        if self.find_num_for_cell(cell) != 0:
            return 
        for v in range(-1, 2):
            for k in range(-1, 2):
                x_check, y_check = x + k, y + v
                ...
                if x_check >= self.width or y_check >= self.height:
                    continue
                if self.board[y_check][x_check] == -1:
                    if self.find_num_for_cell((x_check, y_check)) == 0:
                        lst1.append((x_check, y_check))
        if lst1 == list():
            return
        else:
            for i in lst1:
